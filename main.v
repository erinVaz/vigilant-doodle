...
{
    switch (sw & 0x3) {
        case 0x0: xil_printf("[MODE] sw=00 -> BYPASS (passthrough)\r\n"); break;
        case 0x1: xil_printf("[MODE] sw=01 -> SINGLE-PIXEL filter mode\r\n"); break;
        case 0x2: xil_printf("[MODE] sw=10 -> MULTI-PIXEL filter mode (3x3)\r\n"); break;
        case 0x3: xil_printf("[MODE] sw=11 -> EXTRA FEATURES mode\r\n"); break;
    }
}

void print_btn_mode(u8 sw, u8 btn)
{
    btn &= 0xF;
    sw  &= 0x3;
...
{
    u8 sw  = (u8)(XGpio_DiscreteRead(&sw_btn_gpio, SW_CHANNEL)  & 0x3);
    u8 btn = (u8)(XGpio_DiscreteRead(&sw_btn_gpio, BTN_CHANNEL) & 0xF);

    if (sw != prev_sw || btn != prev_btn) {
        xil_printf("\r\n========================================\r\n");

        if (sw != prev_sw)
            print_sw_mode(sw);

        print_btn_mode(sw, btn);
